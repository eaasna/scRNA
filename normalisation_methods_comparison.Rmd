...
```{r}
plot_variable <- function(seu, n){
  top10 <- head(VariableFeatures(seu), n)
...
```{r}
integrated <- NormalizeData(integrated, normalization.method = "LogNormalize")
...
library(cowplot)
plot_grid(plot_variable(log, 10), plot_variable(sct, 10), plot_variable(integrated, 10), ncol = 3)
```
